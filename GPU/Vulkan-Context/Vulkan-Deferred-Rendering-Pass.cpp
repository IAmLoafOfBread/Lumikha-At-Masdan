...
void GPUFixedContext::build_deferredRenderingPass(void) {
	{
		VkAttachmentDescription Descriptions[DEFERRED_RENDERING_ATTACHMENT_COUNT] = { { 0 } };
		{
			...
			for(uint32_t i = 0; i < DEFERRED_RENDERING_ATTACHMENT_COUNT; i++) {
				...
				Descriptions[i].initialLayout = i == DEFERRED_RENDERING_ATTACHMENT_DEPTH_INDEX ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
				if(i < DEFERRED_RENDERING_ATTACHMENT_DEPTH_INDEX) Descriptions[i].finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
				if(i == DEFERRED_RENDERING_ATTACHMENT_DEPTH_INDEX) Descriptions[i].finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
				if(i == DEFERRED_RENDERING_ATTACHMENT_PRESENT_INDEX) Descriptions[i].finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
			}
		}
		
		VkAttachmentReference TotalReferences[DEFERRED_RENDERING_ATTACHMENT_COUNT] = { { 0 } };
		for(uint32_t i = 0; i < DEFERRED_RENDERING_ATTACHMENT_COUNT; i++) {
			TotalReferences[i].attachment = i;
			TotalReferences[i].layout = i == DEFERRED_RENDERING_ATTACHMENT_DEPTH_INDEX ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		}
		
		VkAttachmentReference InputReferences[LIGHTING_PASS_INPUT_COUNT] = { { 0 } };
